...
class ApiClient {
  ...
  Future<Map<String, dynamic>> _request(String method, String path, {Map<String, dynamic>? body}) async {
    ...
    switch (method) {
      case 'GET': response = await _client.get(uri, headers: _headers);
      case 'POST': response = await _client.post(uri, headers: _headers, body: encoded);
  ...
  Future<Map<String, dynamic>> unlockTowerRoom(int roomNumber, double unlockCost) => _request('POST', '/tower/unlock', body: {'roomNumber': roomNumber, 'unlockCost': unlockCost});
