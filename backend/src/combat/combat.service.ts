...
export class CombatService {
  ...
  async attack(attackerId: string, defenderId: string): Promise<BattleResult> {
    ...
    await this.ruleService.validateCombatAttack({ attackerId, defenderId, attackType, energyCost: COMBAT_DEFAULTS.energyCost });
    ...
    await this.energyService.consumeEnergy(attackerId, COMBAT_DEFAULTS.energyCost, result.battleId);
    if (result.success && result.cashStolen > 0) {
      await this.walletService.transferCash(defenderId, attackerId, result.cashStolen, result.battleId);
    }
    this.eventBus.emit('combat.battle.completed', result);
    return result;
  }
  ...
  async calculateWinProbability(attackerId: string, defenderId: string): Promise<number> {
    ...
    const [attackerStats, defenderStats] = await Promise.all([
      this.statService.getEffectiveStats(attackerId),
      this.statService.getEffectiveStats(defenderId),
    ]);
    const attackerPower = this.calculatePower(attackerStats.attack, attackerStats.speed, attackerStats.luck);
    const defenderPower = this.calculatePower(defenderStats.defense, defenderStats.speed, defenderStats.luck);
