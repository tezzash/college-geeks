...
export class EnergyService {
  ...
  async regenerate(playerId: string): Promise<EnergyState> {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const maxEnergy = this.configService.getMaxEnergy();
      const regenerated = this.calculateRegeneratedEnergy(player, maxEnergy, new Date());
  ...
  async getRemainingRegenTime(playerId: string): Promise<number> {
    ...
    const intervalSeconds = this.configService.getEnergyRegenIntervalSeconds();
  ...
  private calculateRegeneratedEnergy(player: Player, maxEnergy: number, now: Date): Pick<Player, 'energy' | 'lastEnergyUpdate'> {
