...
export class SimulationService {
  ...
  simulateEconomy(
    ...
    const totalMoneyCreated = this.applyJobIncome(
      workingPlayers,
      days,
      jobsPerDay,
    );
    const pvp = this.simulatePvpEconomy(
      workingPlayers,
      attacksPerDay * days,
      options,
    );
    return this.createEconomyReport(
      pvp.players,
      totalMoneyCreated,
      pvp.totalMoneyTransferred,
    );
  }
  ...
  simulatePvpEconomy(
    ...
    if (players.length < 2 && attacks > 0) {
      throw new Error(
        "At least two players are required to simulate PvP attacks.",
      );
    }
    ...
    for (let attack = 0; attack < attacks; attack += 1) {
      ...
      if (random() < this.calculateWinProbability(attacker, defender)) {
        const transfer = Math.min(
          defender.cash,
          this.calculateCashTransfer(defender),
        );
  ...
  simulateAllyBalance(options: SimulationOptions = {}): AllyBalanceResult[] {
    const random = this.createRandom(options.seed);
    const records = DEFAULT_ALLIES.map((ally) => ({
      ally,
      wins: 0,
      losses: 0,
      cash: 0,
    ...
    for (let i = 0; i < DEFAULT_ALLIES.length; i += 1) {
      for (let j = i + 1; j < DEFAULT_ALLIES.length; j += 1) {
        ...
        records[firstWins ? i : j].cash += this.calculateCashTransfer(
          DEFAULT_ALLIES[firstWins ? j : i],
        );
      }
    }
  ...
  private applyJobIncome(
    ...
    for (const player of players) {
      const created =
        (player.jobRewardCash ?? DEFAULT_JOB_REWARD) * jobsPerDay * days;
      player.cash += created;
  ...
  private validateStats(stats: BattleStats, name: string): void {
    if (
      !stats ||
      !Number.isFinite(stats.power) ||
      !Number.isFinite(stats.smartness) ||
      stats.power < 0 ||
      stats.smartness < 0
    ) {
      throw new Error(
        `${name} must include non-negative finite power and smartness.`,
      );
    }
  }
  ...
  private validatePlayers(players: EconomyPlayer[]): void {
    ...
    players.forEach((player, index) => {
      ...
      if (!player.id || !Number.isFinite(player.cash) || player.cash < 0) {
        throw new Error(
          `players[${index}] must include an id and non-negative finite cash.`,
        );
      }
    });
  }
  ...
  private calculateWinProbability(
    ...
    return Math.min(
      0.95,
      Math.max(0.05, attackerScore / (attackerScore + defenderScore)),
    );
  }
  ...
  private createEconomyReport(
    ...
    let totalCash = 0;
    let poorestPlayer = players[0];
    let richestPlayer = players[0];

    for (let i = 0; i < players.length; i++) {
      const player = players[i];
      totalCash += player.cash;
      if (player.cash < poorestPlayer.cash) {
        poorestPlayer = player;
      }
      if (player.cash > richestPlayer.cash) {
