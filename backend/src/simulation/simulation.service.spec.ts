...
describe("SimulationService", () => {
  ...
  it("returns deterministic battle results for the same seed", () => {
    const first = service.simulateBattle(attacker, defender, 10_000, {
      seed: 12345,
    });
    const second = service.simulateBattle(attacker, defender, 10_000, {
      seed: 12345,
    });
    assert.deepEqual(
  ...
  it("rejects invalid battle inputs", () => {
    assert.throws(
      () => service.simulateBattle({ power: -1, smartness: 0 }, defender, 1),
      /attackerStats/,
    );
    assert.throws(
      () => service.simulateBattle(attacker, defender, 0),
      /iterations/,
    );
  });
  ...
  it("rejects invalid economy inputs", () => {
    ...
    assert.throws(
      () => service.simulateEconomy(players(), -1, 1, 1),
      /jobsPerDay/,
    );
  ...
  it("allows zero jobs per day", () => {
    assert.equal(
      service.simulateJobIncome(players(), 2, { jobsPerDay: 0 })
        .totalMoneyCreated,
      0,
    );
  });
  ...
  it("transfers existing money without creating cash", () => {
    ...
    assert.ok(result.totalMoneyTransferred > 0);
    assert.equal(
      Math.round(result.averagePlayerCash * 100) / 100,
      Math.round((3_500 / 3) * 100) / 100,
    );
  });

  it("requires two players when attacks are requested", () => {
    assert.throws(
      () => service.simulatePvpEconomy([players()[0]], 1),
      /At least two players/,
    );
    assert.equal(
      service.simulatePvpEconomy([players()[0]], 0).totalMoneyTransferred,
      0,
    );
  });
  ...
  it("returns a balance report for each built-in ally", () => {
    ...
    assert.ok(
      result.every(
        (ally) => Math.abs(ally.winPercent + ally.lossPercent - 100) < 0.000001,
      ),
    );
  });
});
