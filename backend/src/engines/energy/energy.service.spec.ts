...
describe('EnergyService', () => {
  ...
  let configService: Pick<ConfigService, 'getNumber'>;
  ...
  beforeEach(() => {
    ...
    configService = {
      getNumber: jest.fn(async (key: string) => {
        if (key === 'player.energy.max') return 10;
        if (key === 'player.energy.regen_seconds') return 420;
        return 0;
      })
    };
    service = new EnergyService(prisma as PrismaService, configService as unknown as ConfigService);
  });
