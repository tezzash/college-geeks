...
function buildService(overrides: { balance?: number; energy?: number; config?: any; prisma?: Record<string, unknown>; stats?: unknown } = {}) {
  const config = overrides.config ?? { 'gameplay.pvp.energy_cost': 1, 'gameplay.pvp.enabled': true, 'gameplay.tower.max_rooms': 10, 'gameplay.jobs.energy_cost': 2, 'gameplay.jobs.enabled': true };
  const prisma = {
  ...
  return {
    service: new RuleService(
      {
        getNumber: jest.fn((key: string) => Promise.resolve(config[key] ?? 0)),
        getBoolean: jest.fn((key: string) => Promise.resolve(config[key] ?? false))
      } as never,
      { getBalance: jest.fn(() => Promise.resolve({ cash: overrides.balance ?? 1000 })) } as never,
      { getCurrent: jest.fn(() => Promise.resolve({ current: overrides.energy ?? 10 })) } as never,
      { calculate: jest.fn(() => Promise.resolve(overrides.stats ?? { power: 1, smartness: 1 })) } as never,
      prisma,
    ),
...
describe('RuleService', () => {
  describe('canAttack', () => {
    ...
    it('rejects disabled pvp before querying mutable engines', async () => {
      const { service } = buildService({ config: { 'gameplay.pvp.energy_cost': 1, 'gameplay.pvp.enabled': false, 'gameplay.tower.max_rooms': 10, 'gameplay.jobs.energy_cost': 0, 'gameplay.jobs.enabled': true } });
